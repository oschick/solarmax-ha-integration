...
# Load protocol.py directly to avoid importing homeassistant
...
calculate_checksum = mod.calculate_checksum
parse_response = mod.parse_response
verify_frame_checksum = mod.verify_frame_checksum
...
cs = calculate_checksum("FB;01;17|64:PAC|")
print(f"Checksum test: {cs}")

# Test build_request
req = build_request(ADDRESS, ["PAC", "UDC"])
...
crc = calculate_checksum(resp_data)
...
typ_crc = calculate_checksum(typ_resp_data)
typ_mock = "{" + typ_resp_data + typ_crc + "}"
typ_result = parse_response(typ_mock)
...
na_crc = calculate_checksum(na_resp_data)
na_mock = "{" + na_resp_data + na_crc + "}"
na_result = parse_response(na_mock)
