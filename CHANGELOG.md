...
### Added
- **Configurable twilight elevation threshold**: the sun elevation (in degrees) below which the inverter is expected to be offline during dusk/dawn can now be adjusted via the integration's config/options flow instead of being hardcoded, with matching documentation in the README.
